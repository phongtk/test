...
//		}
	...
	for (var i = 0; i < periods.length; i++) {
		var keys = Object.keys(periods[i]);
		str_opt_period += "<option value='" + keys[0] + "'>" + periods[i][keys[0]] + "</option>\n";
	}
	...
	var methods = {
		init: function (opts) {
			...
			block["dom"] = $("<span class='cron-block cron-block-dom'>"
							+ " ngày <select name='cron-dom'>" + str_opt_dom
							+ "</select> </span>")
							.appendTo(this)
							.data("root", this);
			...
			block["mins"] = $("<span class='cron-block cron-block-mins'>"
							+ " vào <select name='cron-mins'>" + str_opt_mih
							+ "</select> phút </span>")
							.appendTo(this)
							.data("root", this);
			...
			block["dow"] = $("<span class='cron-block cron-block-dow'>"
							+ " vào <select name='cron-dow'>" + str_opt_dow
							+ "</select> </span>")
							.appendTo(this)
							.data("root", this);
			...
			block["time"] = $("<span class='cron-block cron-block-time'>"
							+ "vào <select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
